...
function Training() {
  ...
  const titledList = getItems(t, "training.trainingsList").map((item) =>
    item.split(":")
  );
  ...
  return (
    <Section id="training" classes="background-colored-logo" observe={true}>
      <Row classes="py-12 ">
        ...
        <Heading
          type={3}
          classes="text-primary-100 text-center text-balance mb-6"
        >
          {t("subTitle")}
        </Heading>
        <ul className="list-decimal bg-neutral-400/60 p-6 md:px-12 md:py-8 flex flex-col gap-4 mb-6">
          {titledList.map((item) => (
            <li key={item}>
